...
import CrewModal from "../../components/dashboard/CrewModal";

export default function DashboardCrewPage() {
  ...
  const fetchCrew = useCallback(async () => {
    ...
    setError(null);
    try {
      const { data } = await api.get("/crew/all");
      const list = Array.isArray(data?.data) ? data.data : Array.isArray(data) ? data : data?.crew ?? [];
      setCrew(list);
    } catch (err) {
  ...
  const handleRestore = async (item) => {
    const id = getId(item);
    try {
      await api.put(`/crew/${id}`, { isDeleted: false });
    ...
    <div className="p-4 lg:p-8">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6 lg:mb-8">
        <div>
          <h1 className="font-sans-cond uppercase tracking-subheading text-xl lg:text-2xl text-white">
            <span className="opacity-40 font-bold mr-2">Crew</span>
          </h1>
              ...
              <table className="w-full text-left">
                <thead>
                  <tr className="border-b border-white/10">
                    ...
                    <th className="font-sans-cond uppercase tracking-nav text-xs text-space-accent py-4 px-4 lg:px-6">Bio</th>
                  ...
                  {crew.length === 0 ? (
                    <tr>
                      <td colSpan={6} className="p-0">
                        <div className="border border-white/10 bg-white/5 rounded-xl mx-4 my-6 p-10 flex flex-col items-center justify-center text-center">
                          ...
                          <p className="font-sans text-white/60 text-sm">No crew yet. Add one to get started.</p>
                        </div>
                      </td>
                    </tr>
                  ) : (
                    crew.map((item) => {
                      ...
                      >
                        ...
                        <td className="font-sans text-space-accent py-3 px-4 lg:px-6 text-sm">{item.role ?? "—"}</td>
                        <td className="py-3 px-4 lg:px-6 max-w-xs">
                          <div className="font-sans text-space-accent/80 text-sm line-clamp-2 truncate" title={item.bio}>
                            {item.bio ?? "—"}
                          </div>
                        </td>
                        <td className="py-3 px-4 lg:px-6">
            ...
            {crew.length === 0 ? (
              ...
              crew.map((item) => {
                ...
                >
                  ...
                  <p className="font-sans text-space-accent text-sm mt-0.5">{item.role ?? "—"}</p>
                  <p className="font-sans text-space-accent/80 text-sm mt-1 line-clamp-2">{item.bio ?? "—"}</p>
                  <p className="mt-2">
                    <StatusBadge status={item.status} isDeleted={deleted} />
                  </p>
      ...
      {crewToDelete && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          >
            <div className="p-6">
              <h2 id="delete-crew-modal-title" className="font-sans-cond uppercase tracking-subheading text-xl text-white mb-2">
                Delete Crew Member?
              </h2>
              <p className="font-sans text-space-accent/90 text-sm leading-relaxed">
                Are you sure you want to delete <span className="text-white font-medium">{crewToDelete.name ?? "this crew member"}</span>? This action cannot be undone.
              </p>
