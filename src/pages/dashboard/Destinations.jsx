...
export default function Destinations() {
  ...
  const fetchDestinations = useCallback(async () => {
    ...
    try {
      ...
      const list = Array.isArray(data?.data) ? data.data : Array.isArray(data) ? data : data?.destinations ?? [];
  ...
  const handleConfirmDelete = async () => {
    ...
    const id = getId(destinationToDelete);
    setDeleting(true);
    try {
  ...
  const handleRestore = async (dest) => {
    const id = getId(dest);
    try {
      await api.put(`/destinations/${id}`, { isDeleted: false });
    ...
    <div className="p-4 lg:p-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6 lg:mb-8">
        <div>
          <h1 className="font-sans-cond uppercase tracking-subheading text-xl lg:text-2xl text-white">
            <span className="opacity-40 font-bold mr-2">Destinations</span>
          </h1>
          <p className="font-sans text-space-accent/80 text-sm mt-1">
            Manage space destinations
          </p>
        </div>
        <button
          type="button"
              ...
              <table className="w-full text-left">
                <thead>
                  <tr className="border-b border-white/10">
                    ...
                    <th className="font-sans-cond uppercase tracking-nav text-xs text-space-accent py-4 px-4 lg:px-6">Distance</th>
                    <th className="font-sans-cond uppercase tracking-nav text-xs text-space-accent py-4 px-4 lg:px-6">Travel Time</th>
                    ...
                    <tr>
                      <td colSpan={7} className="p-0">
                        <div className="border border-white/10 bg-white/5 rounded-xl mx-4 my-6 p-10 flex flex-col items-center justify-center text-center">
                          ...
                          <p className="font-sans text-white/60 text-sm">No destinations yet. Add one to get started.</p>
                        </div>
                      </td>
                    </tr>
                  ) : (
                    destinations.map((dest) => {
                      const deleted = dest?.isDeleted === true;
                      return (
                      <tr
                        key={getId(dest)}
                      ...
                      >
                        ...
                        <td className="font-sans text-white py-3 px-4 lg:px-6">{dest.name ?? "—"}</td>
                        <td className="py-3 px-4 lg:px-6 max-w-xs">
                          <div className="font-sans text-space-accent/80 text-sm line-clamp-2 truncate" title={dest.description}>
                            {dest.description ?? "—"}
                          </div>
                        </td>
                        <td className="font-sans text-space-accent py-3 px-4 lg:px-6 text-sm">{dest.distance ?? "—"}</td>
                        <td className="font-sans text-space-accent py-3 px-4 lg:px-6 text-sm">{dest.travel ?? dest.travelTime ?? "—"}</td>
                        <td className="py-3 px-4 lg:px-6">
                          <StatusBadge status={dest.status} isDeleted={deleted} />
                        </td>
              ...
              destinations.map((dest) => {
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    <h3 className="font-serif text-white text-lg uppercase">{dest.name ?? "—"}</h3>
                  </div>
                  <p className="font-sans text-space-accent/80 text-sm mt-1 line-clamp-2">{dest.description ?? "—"}</p>
                  <p className="font-sans text-space-accent text-sm mt-1">
                    {dest.distance ?? "—"} · {dest.travel ?? dest.travelTime ?? "—"}
                  </p>
                  <p className="mt-2">
                    <StatusBadge status={dest.status} isDeleted={deleted} />
                  </p>
                  <div className="flex gap-2 mt-4">
                    <button
                      ...
                      onClick={() => handleEdit(dest)}
                      disabled={deleted}
                      className="font-sans-cond uppercase tracking-nav bg-white/5 text-space-accent hover:bg-white/10 hover:text-white px-3 py-1.5 rounded-lg transition-all text-sm  disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                      Edit
                    </button>
                    {deleted ? (
      ...
      {destinationToDelete && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          >
            <div className="p-6">
              <h2 id="delete-modal-title" className="font-sans-cond uppercase tracking-subheading text-xl text-white mb-2">
                Delete Destination?
              </h2>
              <p className="font-sans text-space-accent/90 text-sm leading-relaxed">
                Are you sure you want to delete <span className="text-white font-medium">{destinationToDelete.name ?? "this destination"}</span>? This action cannot be undone.
              </p>
